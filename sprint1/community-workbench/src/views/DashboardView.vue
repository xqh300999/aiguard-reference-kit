...
async function loadDashboard() {
  ...
  try {
    const [pending, processing, resolved, all] = await Promise.all([
      getAlertsApi({ status: 'PENDING', page: 1, size: 5 }),
      getAlertsApi({ status: 'PROCESSING', page: 1, size: 1 }),
      getAlertsApi({ status: 'RESOLVED', page: 1, size: 1 }),
      getAlertsApi({ page: 1, size: 100 }),
    ])
