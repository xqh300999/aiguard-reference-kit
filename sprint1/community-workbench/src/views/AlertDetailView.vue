...
import { getAlertDetailApi } from '@/api/alert'
import { createDispatchApi, getDispatchByAlertApi, updateDispatchApi } from '@/api/dispatch'
import { getElderlyDetailApi } from '@/api/elderly'
import { useAuthStore } from '@/stores/auth'
import type { AlertDetail, DispatchResult, ElderlySummary } from '@/types/api'
import {
  alertStatusTag,
  alertTypeTone,
  dispatchResultText,
...
async function loadDetail() {
  loading.value = true
  try {
    const detail = await getAlertDetailApi(props.id)
    alert.value = detail
    fallbackElderly.value = detail.elderly ?? null

    if (!detail.elderly) {
      fallbackElderly.value = await getElderlyDetailApi(detail.elderlyId).catch(() => null)
    }
  } finally {
    loading.value = false
  }
...
async function submitProcess() {
  ...
  actionLoading.value = true
  try {
    const dispatch = alert.value.dispatch ?? (await getDispatchByAlertApi(alert.value.id))
    await updateDispatchApi(dispatch.id, {
