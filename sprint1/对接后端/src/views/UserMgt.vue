    ...
    <el-dialog :title="isEdit ? '编辑用户' : '新增用户'" v-model="dialogVisible" width="500px">
      <el-form :model="form" :rules="rules" ref="formRef" label-width="100px">
        ...
        <el-form-item label="密码" prop="password">
          <el-input v-model="form.password" type="password" :placeholder="isEdit ? '请输入新密码' : '请输入密码'" />
        </el-form-item>
        ...
        <el-form-item label="社区" prop="communityId">
          <el-select v-model="form.communityId" placeholder="请选择社区" :disabled="form.role === 'ADMIN' || form.role === 'SUPER_ADMIN'">
...
const form = reactive({
  ...
  password: '',
  phone: '',
  role: 'WORKER' as 'SUPER_ADMIN' | 'ADMIN' | 'WORKER' | 'FAMILY' | 'ELDERLY',
  communityId: 0
})
...
const handleSubmit = async () => {
  ...
  await formRef.value.validate(async (valid) => {
    ...
    loading.value = true
    try {
      const data: Partial<User> & { password?: string } = { username: form.username, realName: form.realName, phone: form.phone, role: form.role, communityId: form.role === 'ADMIN' || form.role === 'SUPER_ADMIN' ? undefined : form.communityId }
