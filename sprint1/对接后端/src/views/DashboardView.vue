...
async function loadStats() {
  ...
  const [overviewResult, trendResult, careResult] = await Promise.allSettled([
  ...
  if (overviewResult.status === 'fulfilled') {
    stats.value = overviewResult.value
  } else {
    ElMessage.warning('统计总览接口暂不可用')
  }

  alertTrend.value = trendResult.status === 'fulfilled' ? trendResult.value : []
  careStats.value = careResult.status === 'fulfilled'
    ? careResult.value
    : { totalPlans: 0, completedLogs: 0, completionRate: 0, records: [] }
}
...
async function handleExport() {
  ...
  try {
    ...
    ElMessage.warning('后端暂未提供统计导出接口')
  } finally {
